...
export function AppSidebar() {
  ...
  const isActive = (href: string) => mounted && pathname?.replace(/\/$/, "") === href;
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
        {navigationConfig.map((section, idx) => {
          ...
          return (
            <div key={idx} className="flex flex-col animate-in fade-in slide-in-from-left-2" style={{ animationDelay: `${idx * 100}ms` }}>
              {isCollapsible ? (
                <button
                  ...
                  className={`flex items-center justify-between px-3 py-2 text-xs font-bold uppercase tracking-widest rounded-xl transition-all duration-300 ${
                    isHighlighted ? "text-amber-500 hover:bg-amber-500/10" : "text-slate-400 hover:bg-slate-800/50 hover:text-slate-200"
                  }`}
                >
                ...
                <h3 className={`flex items-center gap-3 px-3 py-2 text-xs font-bold uppercase tracking-widest mb-1 ${
                  ...
                  {section.items.map((link) => {
                    ...
                    return (
                      ...
                      >
                        {link.label}
                        {active && (
                          <div className={`absolute right-2 w-1.5 h-1.5 rounded-full animate-pulse ${isHighlighted ? "bg-amber-500" : "bg-indigo-500"}`} />
                        )}
                      </Link>
                    );
                  })}
